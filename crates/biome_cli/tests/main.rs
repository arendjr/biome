...
mod main {
    ...
    fn unknown_command() {
        ...
        let result = run_cli(fs, &mut console, Args::from([("unknown")].as_slice()));
    ...
    fn empty_arguments() {
        ...
        let result = run_cli(fs, &mut console, Args::from([("format")].as_slice()));
...
mod configuration {
    ...
    fn ignore_globals() {
        ...
        let result = run_cli(fs, &mut console, Args::from(&["check", "file.js"]));
    ...
    it("uses React", () => {
        ...
        let result = run_cli(fs, &mut console, Args::from(&["lint", "tests/test.js"]));
...
pub(crate) fn run_cli<'app>(
    ...
    let workspace = workspace::client(transport, Box::new(fs)).unwrap();
