...
#[test]
fn test_resolve_type_of_destructured_field_of_union_between_interfaces() {
    let mut fs = MemoryFileSystem::default();
    fs.insert(
        "/src/index.ts".into(),
        r#"type FullConfiguration = InternalConfiguration & PublicConfiguration;
...
"#,
    ...
    //assert!(
    //    use_swr_config_ty.is_function_with_return_type(
    //        |ty| ty.has_variant(|variant_ty| variant_ty.is_interface())
    //    )
    //);
    ...
    let snapshot =
        ModuleGraphSnapshot::new(module_graph.as_ref(), &fs).with_resolver(resolver.as_ref());
    snapshot.assert_snapshot("test_resolve_type_of_destructured_field_of_union_between_interfaces");
}
