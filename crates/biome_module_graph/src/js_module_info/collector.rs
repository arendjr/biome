...
impl JsModuleInfoCollector {
    ...
    fn purge_redundant_types(&mut self) {
        ...
        let (types, map, num_removed_types_at_index) = types.deduplicated(TypeResolverLevel::Thin);
        self.types = types;

        if self.types.len() == num_types {
            return; // Nothing more to do.
        }

        let update_resolved_id = |resolved_id: &mut ResolvedTypeId| {
            if resolved_id.level() == TypeResolverLevel::Thin {
                ...
                *resolved_id =
                    ResolvedTypeId::new(TypeResolverLevel::Thin, TypeId::new(new_index as usize));
            }
        };
