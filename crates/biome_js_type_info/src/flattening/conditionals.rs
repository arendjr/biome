...
pub fn reference_to_falsy_subset_of(
    ...
    let filter = |ty: &TypeData| match ty {
        TypeData::BigInt => FilteredData::Mapped(Literal::BigInt(Text::const_new("0n")).into()),
        TypeData::Boolean => FilteredData::Mapped(Literal::Boolean(false.into()).into()),
        TypeData::Number => {
            FilteredData::Mapped(Literal::Number(NumberLiteral::new(Text::const_new("0"))).into())
        }
        TypeData::String => {
            FilteredData::Mapped(Literal::String(Text::const_new("").into()).into())
        }
        other => {
